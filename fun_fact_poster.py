...
def summarise_article(
    ...
    for attempt, char_limit in enumerate(SUMMARY_CHAR_LIMITS[:MAX_SUMMARY_ATTEMPTS], start=1):
        ...
        if len(summary) > char_limit:
            logging.info(
                "Summary exceeded requested limit (%s > %s); trimming.",
                len(summary),
                char_limit,
            )
            summary = truncate_text(summary, char_limit)

        hashtags_list = _sanitise_hashtags(raw_hashtags)
        if not hashtags_list and fallback_hashtags:
            hashtags_list = list(fallback_hashtags)
